        ...
        if (FD_ISSET(server_fd, &fds)) {
            ...
            if (r > 0) {
                ...
                if (strcmp(ident, "S1") == 0) {
                    ...
                    human_admin.socket = ns;
                    strncpy(human_admin.vardas, ident, 49);
                    human_admin.nustatytas = 1;
                    send(ns, "VARDASOK\n", 9, 0);
                    printf("[LOG] Admin User '%s' joined.\n", human_admin.vardas);
                }
            }
        }
        ...
        if(human_admin.socket > 0 && FD_ISSET(human_admin.socket, &fds)) {
            ...
            if(av <= 0) { human_admin.socket = -1; }
                ...
                if(strncmp(abuf, "#stop", 5) == 0) {
                    ...
                    char conf[100];
                    snprintf(conf, 100, "SYSTEM: %s banned and messages cleared.\n", target);
                    send(human_admin.socket, conf, strlen(conf), 0);
                }
            }
        }
        ...
        for(int i = 1; i <= 2; i++) {
            if(node_sockets[i] > 0 && FD_ISSET(node_sockets[i], &fds)) {
                ...
                if(sscanf(buf, "%[^|]|%[^|]|%[^\n]", f, t, c) == 3) {
                    // Check if banned
                    int is_banned = 0;
                    for(int b=0; b<banned_count; b++) if(strcmp(f, banned[b])==0) is_banned=1;
                    ...
                    if(!is_banned) {
                        ...
                        if(human_admin.socket > 0) {
                            ...
                            snprintf(logmsg, BUFFER_SIZE, "[WAITING 10S] %s -> %s: %s\n", f, t, c);
...
void run_node(int my_port, int admin_port, char *id) {
    ...
    char name_req[50];
    recv(admin_fd, name_req, 49, 0); // "ATSIUSKVARDA"
    send(admin_fd, id, strlen(id), 0); // Reply with "S1" or "S2"
    ...
    while(1) {
        ...
        if(FD_ISSET(admin_fd, &fds)) {
            ...
            if(v > 0) { b[v] = '\0'; for(int i=0; i<MAX_CLIENTS; i++) if(clients[i].socket > 0 && clients[i].nustatytas) send(clients[i].socket, b, strlen(b), 0); }
        }
